...
export function EvolucaoHorasCard({ evolucao: inicial, ciclosList, hoje }: EvolucaoHorasCardProps) {
    ...
    <Card className="h-full flex flex-col bg-card border-primary/15">
      <CardHeader className="pb-1 pt-3 px-3 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold flex-shrink-0">Evolução de Horas</span>
          {ciclosList.length > 0 && (
        ...
        {loading ? (
          ...
          <ChartContainer config={chartConfig} className="h-full w-full">
            <BarChart data={chartData} margin={{ left: 12, right: 20, top: 16, bottom: 12 }}>
              ...
              <YAxis tickLine={false} axisLine={false} tickMargin={8} tickFormatter={(v) => formatMinutos(Number(v))} />
              <ChartTooltip cursor={false} content={<ChartTooltipContent formatter={(v, n) => [formatMinutos(Number(v)), n]} />} />
