...
export function GraficoLeituraDiaria({ materialId }: GraficoLeituraDiariaProps) {
      ...
      return (
        <div className="bg-white p-3 border rounded-lg shadow-lg">
          ...
          {tipoGrafico === 'tempo' ? (
            ...
            <p className="text-green-600 font-medium">{`Páginas: ${data.paginasLidas}`}</p>
          )}
          <p className="text-xs text-gray-500">{`Página atual: ${data.paginaAtual}`}</p>
        </div>
      )
    }
    return null
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
        ...
        <div className="flex gap-4 text-sm">
          ...
          <div className="text-center">
            <div className="text-lg font-bold text-green-600">
              {Math.max(...dados.map(item => item.paginaAtual), 0)}
            </div>
      ...
      {/* Gráfico */}
      <div className="h-48 animate-chart-scale-in">
          ...
          {tipoGrafico === 'tempo' ? (
            ...
            <BarChart data={dados}>
              ...
              <YAxis 
                ...
                tickLine={{ stroke: '#9CA3AF' }}
                axisLine={{ stroke: '#D1D5DB' }}
                label={{ value: 'Páginas', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: '#6B7280', fontSize: 12 } }}
              />
              <Tooltip content={<CustomTooltip />} />
              <Bar 
